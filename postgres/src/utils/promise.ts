/*
 * chain allow to apply a mapper function and await the returned value for each items, one at a time.
 ...
 */
export const chain = <T, R>(array: T[], mapper: (item: T, index: number) => Promise<R>): Promise<R> =>
  array.reduce(
